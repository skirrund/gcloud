...
func (s *NacosDataSource) Initialize() error {
	...
	if err == nil {
		logger.Info("[Nacos] Nacos data source is successfully initialized", ",group=", s.group, ",dataId=", s.dataId)
	}
	return err
}
...
func (s *NacosDataSource) ReadSource() ([]byte, error) {
	...
	logger.Info("[Nacos] Succeed to read source,", "group=", s.group, ",dataId=", s.dataId, ",content=", content)
...
func (s *NacosDataSource) listen(client config_client.IConfigClient) (err error) {
	listener := vo.ConfigParam{
		...
		OnChange: func(namespace, group, dataId, data string) {
			logger.Info("[Nacos] Receive listened property", ",namespace=", namespace, ",group=", group, ",dataId=", dataId, ",data=", data)
			err := s.doUpdate([]byte(data))
			if err != nil {
				logger.Error(err, "Fail to update data source in NacosDataSource.listen()")
			}
		},
	}
...
func (s *NacosDataSource) Close() error {
	...
	logger.Info("[Nacos] The nacos datasource had been closed", ",group=", s.group, ",dataId=", s.dataId)
	return nil
}
