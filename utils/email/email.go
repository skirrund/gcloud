...
const (
	...
	defaultContentType = "text/plain; charset=UTF-8" // defaultContentType is the default Content-Type according to RFC 2045, section 5.2
)
...
func (e *Email) SendWithTLS(addr string, a smtp.Auth, t *tls.Config) error {
	...
	conn, err := tls.DialWithDialer(&net.Dialer{Timeout: 3 * time.Second}, "tcp", addr, t)
	if err != nil {
		return err
	}
