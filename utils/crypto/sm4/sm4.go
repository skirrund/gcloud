...
import (
	...
	"github.com/skirrund/gcloud/utils/padding"
)
...
func Sm4Cbc(hexKey string, cipherText string, encryptMode bool, cbcIV []byte) (outStr string, err error) {
	key, err := getKey(hexKey)
	if err != nil {
		return outStr, err
	}
	var inData []byte
	if encryptMode {
		inData = padding.PKCS.Padding([]byte(cipherText), BlockSize)
	} else {
		inData, _ = hex.DecodeString(strings.ToUpper(cipherText))
	}
	...
	if encryptMode {
		for i := 0; i < len(inData)/BlockSize; i++ {
			in_tmp := xor(inData[i*BlockSize:i*BlockSize+BlockSize], iv)
		...
		for i := 0; i < len(inData)/BlockSize; i++ {
			in_tmp := inData[i*BlockSize : i*BlockSize+BlockSize]
			out_tmp := make([]byte, BlockSize)
		...
		out = padding.PKCS.UnPadding(out)
		outStr = string(out)
	}
...
func getKey(hexKey string) ([]byte, error) {
	key, _ := hex.DecodeString(strings.ToUpper(hexKey))
	if len(key) != BlockSize {
		return nil, errors.New("SM4: invalid key size " + strconv.Itoa(len(key)))
	}
	return key, nil
...
// func Sm4CFB(key []byte, in []byte, mode bool, cfbIv []byte) (out []byte, err error) {
// 	if len(key) != BlockSize {
// 		return nil, errors.New("SM4: invalid key size " + strconv.Itoa(len(key)))
// 	}
// 	var inData []byte
// 	if mode {
// 		inData = padding.PKCS7Padding(in, BlockSize)
// 	} else {
// 		inData = in
// 	}
...
// 			if i == 0 {
// 				c.Encrypt(K, cfbIv)                                   //这里是加密，而不是调用解密方法Decrypt
...
// 		out, _ = padding.PKCS7UnPadding(out, BlockSize)
// 	}
...
// // https://blog.csdn.net/sinat_23338865/article/details/72869841
// func Sm4OFB(key []byte, in []byte, mode bool, ofbIV []byte) (out []byte, err error) {
// 	if len(key) != BlockSize {
// 		return nil, errors.New("SM4: invalid key size " + strconv.Itoa(len(key)))
// 	}
// 	var inData []byte
...
// 		for i := 0; i < len(inData)/16; i++ {
// 			if i == 0 {
// 				c.Encrypt(K, ofbIV)                                   //这里是加密，而不是调用解密方法Decrypt
// 				plainBlock = xor(K[:BlockSize], inData[i*16:i*16+16]) //获取明文分组
// 				copy(out[i*16:i*16+16], plainBlock)
// 				copy(shiftIV, K[:BlockSize])
// 				continue
