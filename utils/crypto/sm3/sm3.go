...
const (
	BlockSize        = 64
	Size             = 32
	digest0   uint32 = 0x7380166f
	digest1   uint32 = 0x4914b2b9
	digest2   uint32 = 0x172442d7
	digest3   uint32 = 0xda8a0600
	digest4   uint32 = 0xa96f30bc
	digest5   uint32 = 0x163138aa
	digest6   uint32 = 0xe38dee4d
	digest7   uint32 = 0xb0fb0e4e
)
...
func (sm3 *SM3) pad() ([]byte, error) {
	...
	msgLen := len(msg)
	if msgLen%64 != 0 {
		return msg, errors.New("SM3 Pad: error msgLen is not %64")
	}
	return msg, nil
}
...
func (sm3 *SM3) Reset() {
	// Reset digest
	sm3.digest[0] = digest0
	sm3.digest[1] = digest1
	sm3.digest[2] = digest2
	sm3.digest[3] = digest3
	sm3.digest[4] = digest4
	sm3.digest[5] = digest5
	sm3.digest[6] = digest6
	sm3.digest[7] = digest7
...
func Sm3SumHex(data []byte) string {
	var sm3 SM3

	sm3.Reset()
	_, _ = sm3.Write(data)
	return hex.EncodeToString(sm3.Sum(nil))
}
func Sm3SumBase64(data []byte) string {
	var sm3 SM3

	sm3.Reset()
	_, _ = sm3.Write(data)
	return base64.StdEncoding.EncodeToString(sm3.Sum(nil))
}
